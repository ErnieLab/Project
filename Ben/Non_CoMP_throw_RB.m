% ===================================================== %
% 該function是用來讓**Non-CoMP**的UE，根據RSRQ來丟掉RB  %
% ===================================================== %
function [BS_RB_table_output, BS_RB_who_used_output, UE_RB_used_output, UE_throughput_After_throw] = Non_CoMP_throw_RB(n_MC, n_PC, BS_RB_table, BS_RB_who_used, UE_RB_used, AMP_Noise, n_ttoffered, Pico_part, RsrpBS_Watt, ...
										                                                                               idx_UE, Serving_Cell_index, GBR, BW_PRB)
											                                                                           
% ------- %
% Initial %
% ------- %
if Serving_Cell_index <= n_MC
	RB_we_can_throw = find(UE_RB_used(idx_UE, :) == 1);         % 我們可以拿的RB，也就是idx_trgt沒有使用的RB
else
	RB_we_can_throw = find(UE_RB_used(idx_UE, 1:Pico_part) == 1); % 我們可以拿的RB，也就是idx_trgt沒有使用的RB
end
