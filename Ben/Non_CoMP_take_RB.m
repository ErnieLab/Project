% =================================================== %
% 該function是用來讓**Non-CoMP**的UE，根據RSRQ來拿RB  %
% =================================================== %
...
RB_RSRQ = zeros(1, length(RB_we_can_take));             % UE準備跟Serving_Cell_index拿RB，所以該矩陣是Serving_Cell_index中，沒有被使用的RB的RSRQ   
	...
	while UE_throughput < GBR	
		if (isempty(RB_we_can_take) == 1) % 如果RB已經被拿光了	
			...
			if RB_throughput == 0  % 如果拿了RSRQ最高的RB, Throughput居然是0，代表UE離 Serving_Cell_index太遠了		
				...
				BS_RB_table(Serving_Cell_index, RB_we_can_take(RB_maxRSRQ_index))    = 1;      % 把該位置記錄說，有人在用了
				BS_RB_who_used(Serving_Cell_index, RB_we_can_take(RB_maxRSRQ_index)) = idx_UE; % 誰用了該RB 登記起來
