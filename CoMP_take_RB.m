...
RB_empty_SINR = zeros(1, length(RB_empty));                                  % 兩個Cell都沒使用的RB 的SINR
...
if (isempty(RB_empty) == 0) % 有交集進來算
	...
	while UE_throughput < GBR	
		if (isempty(RB_empty) == 1)
			...
			[RB_maxSINR_value, RB_maxSINR_index] = max(RB_empty_SINR);
			...
			if 	RB_throughput == 0 % 如果拿了Throughput最高的RB, Throughput居然是0，代表UE離兩邊Cell都太遠了=>  不玩了??
				...
				BS_RB_table(Serving_Cell_index, RB_empty(RB_maxSINR_index))        = 1;				
				BS_RB_who_used(Serving_Cell_index, RB_empty(RB_maxSINR_index))     = idx_UE;
				BS_RB_table(Cooperating_Cell_index, RB_empty(RB_maxSINR_index))    = 1;
				BS_RB_who_used(Cooperating_Cell_index, RB_empty(RB_maxSINR_index)) = idx_UE;

				UE_RB_used(idx_UE, RB_empty(RB_maxSINR_index)) = 1;

				UE_throughput = UE_throughput + RB_throughput;

				RB_empty_SINR(RB_maxSINR_index) = [];
				RB_empty(RB_maxSINR_index)      = [];
		    end
		end
	end
end
