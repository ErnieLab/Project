...
might_move_these_RB_RSRQ   = zeros(1, length(Target_might_move_these_RB));
...
for RB_index = 1:1:length(Target_might_move_these_RB)  % 這些可以丟的RB，最後要算出每一塊RB的RSRQ
	RB_Total_Interference = 0;
	for BS_index = 1:1:(n_MC + n_PC)
		if BS_index ~= Serving_Cell_index  % 注意這邊
			if BS_index <= n_MC
				if BS_RB_table(BS_index, Target_might_move_these_RB(RB_index)) == 1    % 別的Macro Cell有用到該RB，就要算進來 
				...
				if BS_RB_table(BS_index, Target_might_move_these_RB(RB_index)) == 1    % 別的Pico Cell有用到該RB，就要算進來 
	...
	might_move_these_RB_RSRQ(RB_index) = Serving_Cell_RSRP_watt_perRB*(1/(RB_Total_Interference + Serving_Cell_RSRP_watt_perRB));
end
...
while UE_throughput < GBR
	if isempty(Target_might_move_these_RB) == 1  % 有交集進來算，沒交集代表UE佔的位置，Target   Cell都沒有佔到，或者沒辦法換，也就是今天UE的Throughput不夠，是大環境的問題
		...
		[~, might_move_RB_minRSRQ_index] = min(might_move_these_RB_RSRQ);  % 這個是Target Cell準備要移走的RB

		User_index_occupy_target_might_move_RB = BS_RB_who_used(Target_Cell_index, might_move_these_RB_RSRQ(might_move_RB_minRSRQ_index)); % 佔住準備要移走的RB的UE
		...
		% 要被移動RB的user，放掉該RB
		BS_RB_table(Target_Cell_index, might_move_these_RB_RSRQ(might_move_RB_minRSRQ_index))                     = 0;
		BS_RB_who_used(Target_Cell_index, might_move_these_RB_RSRQ(might_move_RB_minRSRQ_index))                  = 0;
		UE_RB_used(User_index_occupy_target_might_move_RB, might_move_these_RB_RSRQ(might_move_RB_minRSRQ_index)) = 0;
		...
		for Empty_index = 1:1:length(Target_Cell_empty)
			...
			for RB_index = 1:1:length(RB_user_take)
				...
				RB_RSRQ               = user_rsrp_watt_perRB*(1/(RB_Total_Interference + user_rsrp_watt_perRB));
				RB_throuhgput         = BW_PRB*MCS_3GPP36942(RB_RSRQ);
		...
		if user_maxThourghput_value < GBR
			% 換也沒用，RB還來!!!!
			BS_RB_table(Target_Cell_index, might_move_these_RB_RSRQ(might_move_RB_minRSRQ_index))                     = 1;
			BS_RB_who_used(Target_Cell_index, might_move_these_RB_RSRQ(might_move_RB_minRSRQ_index))                  = User_index_occupy_target_might_move_RB;
			UE_RB_used(User_index_occupy_target_might_move_RB, might_move_these_RB_RSRQ(might_move_RB_minRSRQ_index)) = 1;

			% 這個RB試過了，結果是換不了，把該RB剃除UE的選取範圍內
			might_move_these_RB_RSRQ(might_move_RB_minRSRQ_index)   = [];
			Target_might_move_these_RB(might_move_RB_minRSRQ_index) = [];
		else
			...
			for RB_index = 1:1:length(RB_UE_used)  % 這些可以丟的RB，最後要算出每一塊所提供的  Throughput
				...
				for BS_index = 1:1:(n_MC + n_PC)
					if BS_index ~= Serving_Cell_index  % 注意這邊
						if BS_index <= n_MC
							if BS_RB_table(BS_index, RB_UE_used(RB_index)) == 1    % 別的Macro Cell有用到該RB，就要算進來 
								RsrpMC_watt_perRB     = RsrpBS_Watt(BS_index)/n_ttoffered;         % watt在除以RB數目					
								RB_Total_Interference = RB_Total_Interference + RsrpMC_watt_perRB; % 加起來
							end
						else
							if BS_RB_table(BS_index, RB_UE_used(RB_index)) == 1    % 別的Pico Cell有用到該RB，就要算進來 
								RsrpPC_watt_perRB     = RsrpBS_Watt(BS_index)/Pico_part;           % watt在除以RB數目						 
								RB_Total_Interference = RB_Total_Interference + RsrpPC_watt_perRB; % 加起來
							end
						end 
